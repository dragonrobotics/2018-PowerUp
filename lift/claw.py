...
- ``closing``: the claw is transitioning from the "neutral" state to the "closed"
  state.
...
class Claw:
    ...
    def close(self):
        ...
        This function does a quick check to make sure it is a valid state change
        before actually changing the state to "closing".
        For example you cannot set the state to "closing" if the claw is already
        gripping the cube.
        """
